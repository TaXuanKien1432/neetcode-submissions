class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def dfs(row, column, curr, visited, word):
            ...
            down = False
            if row < numOfRows[0] - 1 and (row + 1, column) not in visited:
                visited.add((row + 1, column))
            ...
            right = False
            if column < numOfColumns[0] - 1 and (row, column + 1) not in visited:
                visited.add((row, column + 1))
        ...
        for row in range(numOfRows[0]):
            for column in range(numOfColumns[0]):
